...
# For converting SQLite timestamp strings
...
def dashboard():
    ...
    uid = session["user_id"]
    total = query_db(
        "SELECT COUNT(*) AS c FROM complaints WHERE user_id=?",
        (uid,),
        one=True,
    )["c"]
    pending = query_db(
    ...
    if request.method == "POST":
        ...
        if "photo" in request.files:
            ...
            if file and file.filename != "" and allowed_file(file.filename):
                ...
                flash(
                    "Invalid file type. Allowed: png, jpg, jpeg, gif, pdf",
                    "danger",
                )
                return redirect(url_for("submit_complaint"))
        ...
        query_db(
            ...
            description, category, photo, location)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (
                complaint_id,
...
def my_complaints():
    ...
    return render_template(
        "my_complaints.html", complaints=complaints, status_filter=status
    )
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        admin = query_db(
            "SELECT * FROM admins WHERE email=?", (email,), one=True
        )
        if admin and check_password_hash(admin["password_hash"], password):
            session["admin_id"] = admin["id"]
            session["admin_name"] = admin["email"]
            flash(f"Welcome, Admin {admin['email']}!", "success")
...
def admin_dashboard():
    ...
    pending = query_db(
        "SELECT COUNT(*) AS c FROM complaints WHERE status='Pending'", one=True
    )["c"]
    in_progress = query_db(
...
def admin_complaints():
    ...
    if search:
        sql += (
            " AND (c.complaint_id LIKE ? OR c.title LIKE ? "
            "OR u.name LIKE ? OR c.category LIKE ?)"
        )
        like = f"%{search}%"
...
def admin_update(complaint_id):
    ...
    if request.method == "POST":
        ...
        query_db(
            ...
            WHERE id=?
        """,
            (new_status, dept_id, admin_notes, complaint_id),
            commit=True,
        )
...
"""
    ...
    return render_template(
        "admin_update.html", complaint=complaint, departments=departments
    )
...
def admin_delete(complaint_id):
    ...
    row = query_db(
        "SELECT photo FROM complaints WHERE id=?", (complaint_id,), one=True
    )
...
def admin_report():
    ...
    """)
    grand_total = query_db("SELECT COUNT(*) AS c FROM complaints", one=True)[
        "c"
    ]
    grand_resolved = query_db(
        "SELECT COUNT(*) AS c FROM complaints WHERE status='Resolved'",
        one=True,
    )["c"]
